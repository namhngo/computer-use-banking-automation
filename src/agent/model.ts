...
Call discover only when no catalog entry fulfils the goal AND the goal is to READ information from the application for a record that the goal identifies explicitly; pass those identifiers verbatim as inputs.
Call clarify when the goal is a read but a required identifier is missing or ambiguous, or when two catalog entries could both apply. Ask one short question.
...
const routeTools = {
  ...
  discover: { description: 'No catalog entry fits: let the discovery agent learn this read-only flow from the live UI.', inputSchema: routeToolSchemas.discover },
