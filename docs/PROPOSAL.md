...
- Reviewed real runs are under `evidence/replay-phase3/`. The original run IDs/timestamps are
  ...
  `evidence/discovery-phase4/`, compile/verify under `evidence/compile-phase5/`, real
  attended/unattended handoff runs (with `intervention_1.json`) under `evidence/hitl-phase6/`,
  and live router runs (cold, warm, clarify, refuse, denied) under `evidence/agent-phase7/`.
...
| **8. Evidence set + docs** (completed) | reviewed `evidence/` (replay, discovery, compile, HITL, agent), README (setup, agent and direct commands, offline replay, fresh-clone check), `REPORT.md` with the seven exact headings | Fresh clone: frozen install, `config:check`, typecheck, 795 unit + 172 browser tests, sandbox replays `SUCCESS` and `MEMBER_NOT_FOUND`, `agent` without a key fails closed. Evidence review: no provider key, credential, member value, balance, or operator token in any log/artifact; only the caller's `agent-result.json` files carry outputs; the sole `password` hit is the structural `login_password` key. README marks design-only items as such. |
